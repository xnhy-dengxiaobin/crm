<template>
  <div class="mod-config">
    <el-form :inline="true"
      ...
      <el-form-item>
        <el-date-picker v-model="dataForm.createdTimes"
                        type="datetimerange"
      ...
      <el-form-item>
        <el-select v-model="dataForm.busiStatus"
                   clearable
                   multiple
                   placeholder="客户状态">
          <el-option label="报备"
      ...
      <el-form-item>
        <el-select style="width: 100%"
                   ...
                   filterable
                   placeholder="渠道身份">
          <el-option v-for="middleType in middleTypes"
                     :key="middleType.id"
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('busi:prepare:update')"
                   type="primary"
                   @click="refresh()">刷新保护期</el-button>
    ...
    <el-table :data="dataList"
      ...
      <el-table-column prop="middleTypeName"
                       header-align="center"
                       align="center"
                       label="渠道身份">
      </el-table-column>
      ...
      <el-table-column prop="matchUserName"
                       header-align="center"
                       align="center"
                       label="顾问">
      </el-table-column>
      ...
      <el-table-column fixed="right"
        ...
        <template slot-scope="scope">
          <el-button v-if="isAuth('busi:prepare:update')"
                     type="text"
                     size="small"
                     @click="refresh(scope.row.id)">刷新保护期</el-button>
          <!-- <el-button
...
export default {
  ...
  methods: {
    ...
    refresh (id, status) {
      ...
      this.$confirm(
        `确定对[id=${ids.join(",")}]进行[${id ? "立即执行" : "批量立即执行"
        }]刷新保护期操作?`,
        "提示",
        {
    ...
    exportExcel () {
      ...
      this.dataForm.busiStatus =
        this.dataForm.busiStatus && this.dataForm.busiStatus.length > 0
          ...
          : null;
      this.dataUtil.exec({
        steps: [{ type: "grv", name: "export/ExportPrepareCustomerStep" }],
        dataParam: {
          page: this.pageIndex,
          limit: this.pageSize,
          name: this.dataForm.name,
