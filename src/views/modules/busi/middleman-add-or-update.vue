<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="渠道身份" prop="middleTypeId">
        <el-select
          style="width: 100%"
          v-model="dataForm.middleTypeId"
          @change="handleMiddleTypeSelect"
          filterable
          placeholder="请选择渠道身份"
        >
          <el-option
...
export default {
  data() {
    ...
    return {
      ...
      dataRule: {
        ...
        middleTypeId: [
          { required: true, message: "渠道身份不能为空", trigger: "blur" },
        ],
        mobile: [
  ...
  methods: {
          ...
          if (this.dataForm.id) {
            this.$http({
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.middleTypeName = data.user.middleTypeName;
                this.dataForm.middleTypeId = data.user.middleTypeId;
        ...
        if (valid) {
          this.$http({
            ...
            data: this.$http.adornData({
              ...
              middleTypeName: this.dataForm.middleTypeName,
    ...
    async handleMiddleTypeSelect(item) {
      let o = await this.middleTypes.find((it) => {
        return it.id === item;
      });
      this.dataForm.middleTypeName = o ? o.name : "";
    },
    // 文件上传相关
    handleRemove(file) {
      console.log(file);
